...
const HEADER_SOU = '#layout-body > section > section > div:nth-child(1) > div > div.el-card > div > div:nth-child(2) > div.flex.items-center > div > span'
...
const config = {
  replacements: {
    text: {
      ...
      "飞书": "Telegram",
      "续费": "",
      "download.bt.cn": "v920.hostcli.com",
      "2034-09-21": "2099-12-31",
    },
    href: {
      "www.bt.cn/new/wechat_customer": "tg.hostcli.com",
      "www.bt.cn/bbs/thread-1172-1-1.html": "support.hostcli.com/bt-command/btcode?from=v920.hostcli.com_bt-linux-panel-9.2.0#change-password",
      "www.bt.cn/bbs/thread-115241-1-1.html": "support.hostcli.com/telegram-bot/telegram-bot-notification?from=v920.hostcli.com_bt-linux-panel-9.2.0",
      "www.bt.cn/bbs/thread-108274-1-1.html": "support.hostcli.com/telegram-bot/telegram-bot-notification?from=v920.hostcli.com_bt-linux-panel-9.2.0",
      "www.bt.cn": "v920.hostcli.com",
    },
    src: {
  ...
  links: {
    top: [  // Changed from 'support' to 'top'
      { text: '站長資源', url: 'https://3dayseo.com' },
      { text: '文尼模板網', url: 'https://wntheme.com' }
    ],
    left: [
      {
...
function AddLeft(menuSelector, items) {
    ...
    if (menu){
        items.forEach(item => {
            const listItem = `
                <li data-v-a415c3d7="" class="menu-item menu-soft">
                    <a data-v-a415c3d7="" href="${item.url}" target="_blank" class="flex justify-start !p-[1.4rem] !pl-[1.6rem]">
                                  ${item.svgPath}
                        <span style="color:#feeb00" class="ml-[1rem]">${item.text}</span>
